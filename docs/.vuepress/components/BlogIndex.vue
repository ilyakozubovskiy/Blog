<!-- <template>
	<div id="">
	  ...
	  <div class="card-panel-content row" itemscope itemtype="http://schema.org/Blog">
		...
		<article v-for="page in pages" :key="page.key" itemprop="blogPost" class="ui-post entry col-md-4 card-container"
		  ...
		  <div class="entry card h-100">
			...
			<div class="card-inside">
			  <div class="card-subheading">
				<div v-if="page.frontmatter.tags" class="ui-post-meta ui-post-tag" itemprop="keywords">
				  <router-link v-for="tag in resolvePostTags(page.frontmatter.tags)" :key="tag" :to="'/tag/' + tag">
					{{ tag }}
				  </router-link>
				</div>
			  </div>
			...
			<div class="meta-bottom mt-auto">
				...
				<span v-for="author in $themeConfig.authors" :key="author.name" class="nav-item">
				  <div class="d-flex">
					<a class="profile-avatar">
					  <img v-if="author.name === page.frontmatter.author" :src="($withBase)(author.avatar)"
						class="avatar-image" :alt="author.name">
					</a>
					<div class="meta">
			  ...
			  <div v-if="page.frontmatter.date" class="date"><time pubdate itemprop="datePublished"
